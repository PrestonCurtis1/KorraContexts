...
public class AddonContextLoader<T> {
    private final Plugin plugin;
    private final File directory;
    private final ArrayList<File> files;
    private ClassLoader loader;

    public AddonContextLoader(final Plugin plugin, final File directory) {
        this.plugin = plugin;
        ...
        if (!directory.exists() && !directory.mkdirs()) {
            plugin.getLogger().warning("Could not create addon context folder: " + directory.getAbsolutePath());
            return;
        }
        if (!directory.isDirectory()) {
            plugin.getLogger().warning("Addon context path is not a directory: " + directory.getAbsolutePath());
            return;
        }

        final File[] jarFiles = directory.listFiles(file -> file.isFile() && file.getName().toLowerCase().endsWith(".jar"));
        if (jarFiles == null) {
            plugin.getLogger().warning("Could not read addon context directory: " + directory.getAbsolutePath());
            return;
        }
        ...
        for (final File file : this.files) {
            ...
            try {
                ...
                if (jarFile.getEntry("plugin.yml") != null) {
                    this.plugin.getLogger().log(Level.WARNING, "The JAR file " + file.getName() + " is a plugin and not an context! Put it in the plugins folder!");
                    continue;
                }
